...
<div class="row-fluid">
    <form class="form well form-inline" method="POST" id="formGaji">
        ...
        <div class="row-fluid">
            ...
            <div class="span6">
                <table>
                    <tr>
                        <td class="labelField">Guru</td>
                        <td><?php dropDownDataGuru('name="guru" id="guru" class="guru" required') ?></td>
                    </tr>
                    <tr>
                        <td class="labelField">Tugas</td>
...
<script>
    $(document).ready(function(){
        ...
        $(document).on("keydown", "input,select,textarea", function(event) {
            if (event.keyCode === 13) {
                var formInput = $('#formGaji').find('input,select,textarea, button, checkbox');
...
<script>
    ...
    function getAllDataTrans(){
        ...
        if(tahun.length && triwulan.length){
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('pengeluaran/trans_gaji/getDataTrans') ?>",
    ...
    function generateDataTable(response, showMsg){
        ...
        if (response['data'] !== null) {
            for(var i = 0; i < response['data'].length; i++){
                ...
                strRow += '<td style="text-align:right">'+response['data'][i]['JUMLAH']+'</td>';
                strRow += '</tr>';
                total += parseInt(response['data'][i]['JUMLAH']);
    ...
    function getDataByidTrans(idTrans){
        if(idTrans !== null){
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('pengeluaran/trans_gaji/getDataByIdTrans') ?>",
    ...
    function setDataTrans(response){
        ...
        if(response['data'] !== null){
            ...
            $('#guru').val(response['data']['KODE_GURU']);
    ...
    $(document).on('click', '#btnDelete', function(){
        ...
        if(idTrans.length){
            if(confirm('Yakin akan menghapus data ini')){
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('pengeluaran/trans_gaji/prosesDelete') ?>",
                    data: {idTrans:idTrans},
                    dataType: 'json',
                    beforeSend: function(xhr) {
    ...
    $(document).ready(function() {
        $('#formGaji').validate({
            submitHandler: function(form) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('pengeluaran/trans_gaji/prosesSimpan') ?>",
                    data: $('#formGaji').serialize(),
                    dataType: 'json',
                    beforeSend: function(xhr) {
