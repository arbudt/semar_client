...
<div class="row-fluid">
    <form class="form well form-inline" method="POST" id="formBosk3">
            ...
            <div class="span6">
                <table>
                    ...
                    <tr>
                        ...
                        <td nowrap>
                            <input type="text" name="jumlahPengeluaran" id="jumlahPengeluaran" class="jumlahPengeluaran" data-rule-number value="" required/>
                            &nbsp;
                            <input type="checkbox" name="isInputPPh" id="isInputPPh" class="isInputPPh" value="1"/><label for="isInputPPh">&nbsp;Input PPh</label>
                        </td>
                    </tr>
                </table>
            </div>
...
<script>
    $(document).ready(function(){
        ...
        $(document).on("keydown", "input,select,textarea", function(event) {
            if (event.keyCode === 13) {
                var formInput = $('#formBosk3').find('input,select,textarea, button, checkbox');
...
<script>
    ...
    function enableForm(){
        ...
        $('#isInputPPh').prop('disabled', false);
    }
    ...
    function getAllDataTrans(){
        ...
        if(tahun.length && triwulan.length){
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('bosk3/bosk3/getDataTrans') ?>",
        ...
        if (response['data'] !== null) {
            for(var i = 0; i < response['data'].length; i++){
                ...
                if(response['data'][i]['ID_TRANS'] !== null){
                    ...
                    strRow += '<tr class="rowDataTable" data-id-trans="0">';
                }
    ...
    function getDataByidTrans(idTrans){
        if(idTrans !== null){
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('bosk3/bosk3/getDataByIdTrans') ?>",
    ...
    function setDataTrans(response){
        ...
        if(response['data'] !== null){
            ...
            if(response['data']['INPUT_PPH'] === '1'){
                $('#isInputPPh').prop('checked', true);
            }else{
                $('#isInputPPh').prop('checked', false);
            }
        }
    ...
    function setSaldo(){
        ...
        if(triwulan.length){
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('bosk3/bosk3/getSaldoBos') ?>",
                ...
                success: function(response) {
                    if(response['saldo'] !== null){
                        $('#jumlahPenerimaan').val(response['saldo']);
                    }else{
                        $('#jumlahPenerimaan').val(0);
                    }
                    hitungSisaSaldo();
                }
            });
    ...
    function hitungSisaSaldo(){
        var saldo = $('#jumlahPenerimaan').val();
        if(saldo.length < 1){
            saldo = '0';
        }
        var keluar = $('#jumlahPengeluaran').val();
        if(keluar.length < 1){
            keluar = '0';
        }
        var sisaSaldo = (parseInt(saldo)- parseInt(keluar))
        $('#jumlahSaldo').val(sisaSaldo);
    }
    ...
    $(document).on('click', '#btnDelete', function(){
        ...
        if(idTrans.length){
            if(confirm('Yakin akan menghapus data ini')){
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('bosk3/bosk3/prosesDelete') ?>",
                    data: {idTrans:idTrans},
                    dataType: 'json',
                    beforeSend: function(xhr) {
    ...
    $(document).ready(function() {
        $('#formBosk3').validate({
            submitHandler: function(form) {
                $.ajax({
                    type: "POST",
                    url: "<?php echo site_url('bosk3/bosk3/prosesSimpan') ?>",
                    data: $('#formBosk3').serialize(),
                    dataType: 'json',
                    beforeSend: function(xhr) {
